// ═══════════════════════════════════════════════════════════════════════
// BORDKASSE IJsselmeer 2026 — Apps Script v11
//
// Kompatibel mit xlsx-Layout v10 (keine Layout-Änderungen).
//
// NEU in v11:
// - Menüpunkt "🆕 Neuen Törn starten" (neuenToernStarten):
//   leert Transaktionen, Crew-Daten, Törn-Datum, Schulden, Eingabe + Gutschrift
//   in einem Rutsch — Calc-Formeln in Transaktionen/Bilanz/Auswertung bleiben
//
// NEU in v10:
...
// ════════════════════════════════════════════════════════════════════════
// NEUEN TÖRN STARTEN — leert die Datei für eine neue Crew
// ════════════════════════════════════════════════════════════════════════
function neuenToernStarten() {
  const ui = SpreadsheetApp.getUi();
  const r = ui.alert(
    "🆕 Neuen Törn starten?",
    "Das löscht in DIESER Datei:\n" +
    "  • Alle Transaktionen\n" +
    "  • Crew-Liste in Besatzung\n" +
    "  • Törn-Datum (Start/Ende)\n" +
    "  • Schulden-Übersicht\n" +
    "  • Eingabe + Gutschrift-Maske\n\n" +
    "Tipp: Vorher 'Datei → Kopie erstellen' für ein Archiv des alten Törns.\n\n" +
    "Fortfahren?",
    ui.ButtonSet.YES_NO);
  ...
  // 1. Transaktionen leeren — Daten-Spalten B-U + AH-AI in Zeilen 4-203.
  //    Calc-Formeln in V-AG (P1-P12 Anteile) bleiben unangetastet.
  const wsTx = ss.getSheetByName(TX.SHEET);
  if (wsTx) {
    const numRows = TX.MAX - TX.START + 1;
    wsTx.getRange(TX.START, 2,  numRows, 20).clearContent();   // B (DATUM) bis U (P12 dabei)
    wsTx.getRange(TX.START, 34, numRows, 2).clearContent();    // AH (GUT_VON) bis AI (GUT_AN)
  }

  // 2. Schulden-Tab leeren
  const wsSch = ss.getSheetByName(S.SHEET);
  if (wsSch) {
    wsSch.getRange(S.DATA_START, 1, S.DATA_MAX - S.DATA_START + 1, 5).clearContent();
    wsSch.getRange(S.TIMESTAMP, 1).clearContent();
  }

  // 3. Besatzung-Crew leeren — Formeln in E (Tage), G (Zeitanteil %), K (Hilfsspalte)
  //    bleiben erhalten, weil sie auf B/C/D referenzieren.
  const wsBes = ss.getSheetByName(B.SHEET);
  if (wsBes) {
    wsBes.getRange("B5:B6").clearContent();                     // Törn-Start, -Ende
    wsBes.getRange(B.NAME_START, 2, B.N, 3).clearContent();     // B-D: Name, Ab, Bis
    wsBes.getRange(B.NAME_START, 6, B.N, 1).clearContent();     // F: Alkohol-x
    wsBes.getRange(B.NAME_START, 8, B.N, 1).clearContent();     // H: Hinweis
  }

  // 4. Eingabe + Gutschrift zurücksetzen
  const wsEin = ss.getSheetByName(E.SHEET);
  const wsGut = ss.getSheetByName(G.SHEET);
  if (wsEin) eingabeMaskeLeeren_(wsEin);
  if (wsGut) gutschriftMaskeLeeren_(wsGut);

  ui.alert(
    "✅ Neuer Törn vorbereitet",
    "Datei ist leer. Nächste Schritte:\n\n" +
    "1. Besatzung-Tab öffnen → Törn-Datum + Crew eintragen\n" +
    "2. Erste Ausgabe in Eingabe-Tab erfassen",
    ui.ButtonSet.OK);
}
...
function buttonNeuenToernStarten()   { neuenToernStarten(); }
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem("🆕 Neuen Törn starten",     "neuenToernStarten")
    .addToUi();
