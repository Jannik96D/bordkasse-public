...
- **Self-Service-Kontolöschung (DSGVO Art. 17):** im Profil-Bereich (`/profile` → „Konto löschen") kann jeder User sein Konto selbst löschen. SQL-Function `delete_my_account()` (Migration 0021) anonymisiert die `persons`-Row (display_name → „Ehemaliges Crew-Mitglied", `auth_user_id` NULL), löscht `persons_private`, `trip_statistics_audience` sowie Pre-Trip-Mitgliedschaften ohne Buchungs-Spur. Anschließend wird die `auth.users`-Row über die Admin-API entfernt. **Blocker:** wenn der User in einem aktiven Trip (`end_date >= today`, nicht gepurged) Buchungen hat (paid_by / credit_from / credit_to / transaction_participants), wird die Löschung abgewiesen — sonst zerbricht die Bilanz. Server-Action: `app/profile/actions.ts:deleteMyAccount`.
