...
- **Anzahlungs-Modul** (Migrationen 0023–0028, Spec `docs/prepayments.md`): Mechanik für Yacht-Anzahlungen, die der Skipper Monate vor Törn-Start an die Charteragentur leistet und sich von der Crew rückerstatten lässt.
  ...
  - **Matrix** unter `/trips/[id]/prepayments`: Person × Tranche mit Status-Symbolen (○ offen, ◐ teilweise, ✓ bezahlt, ⏰ überfällig, ⏳ pending). Klick auf Zelle → Modal zur Zahlungs-Erfassung mit Überzahlungs-Splitting auf zweite Tranche. Pro Person-Zeile 🔔 (Erinnerungs-Mail) und 💬 (WhatsApp-Text). Sammelnachricht-Button oben. Charter-Reminder-Banner für den Vorstrecker zeigt, was er an die Agentur noch überweisen muss (mit Überfälligkeits-Indikator). Vorstrecker-Zeile trägt blaues „Streckt vor"-Badge, Bell/WhatsApp-Buttons sind in dieser Zeile deaktiviert.
  - **Vorstrecker:** wer streckt die Yacht-Anzahlung vor (Default = Trip-Skipper, im Wizard editierbar). _Sprachregelung:_ „Vorstrecker" ist der interne Konzept-Name (Code-Identifier `advancer_*`, `requireSkipperAdminOrAdvancer`); in **Endnutzer-Texten (UI/Mail)** wird die Rolle geschlechtsneutral formuliert — Badge „Streckt vor", sonst „die vorstreckende Person" bzw. Verb „vorstrecken". Migration 0024 relaxed `tx_credit_self` für tranche-getaggte Credits, damit der Vorstrecker seinen eigenen Anteil als Selbst-Verrechnung abhaken kann (bilanzneutral). Der Vorstrecker darf abhaken, bestätigen, ablehnen und Erinnerungen verschicken — auch wenn er nicht Trip-Skipper ist (`requireSkipperAdminOrAdvancer` in `lib/auth/authz.ts`).
  ...
  - **Crew-Fälligkeit 3 Tage vor Charter:** `prepayment_tranches.due_date` ist die Charter-Fälligkeit (gegenüber Agentur). Überall, wo die Crew die Frist sieht (Matrix-Header, Crew-Self-View, Erinnerungs-Mail, WhatsApp), wird über `toCrewDueDate()` in `lib/prepayments/dates.ts` ein 3-Tage-Puffer abgezogen. Der Charter-Reminder-Banner zeigt weiterhin das echte Datum. `toCrewDueDate` **clampt** zusätzlich: die Crew-Frist rutscht nicht in die Vergangenheit, solange die Charter-Frist noch aussteht, und liegt nie nach der Charter-Frist selbst (`today` als Parameter überschreibbar für deterministische Tests).
...
**Tests:** Vitest (`__tests__/calc.test.ts` + `schema.test.ts` + `prepayments.test.ts` + `prepayment-dates.test.ts`) gegen S1–S7, das Spec-Szenario „Yacht-Anzahlung mit Kojen" sowie Edge-Cases (An-Bord-0-Anwesende, Pro-Person+Alkohol, An-Alle-N=2, `toCrewDueDate`-Clamp); Playwright (`e2e/smoke.spec.ts`) für öffentliche Routes + Auth-Schutz + Security-Header. **pgTAP** (`webapp/supabase/tests/*.sql`, ausführbar via `pnpm test:db` → `supabase test db`) prüft die echten SQL-Views/Funktionen (`v_transaction_shares`, `v_balances`, `simplify_debts`, `v_prepayment_payments`) inkl. Soft-Delete-Regression — **lokal lauffähig, bewusst NICHT im CI** (siehe entfernter db-lint-Job: `supabase start` läuft im GitHub-Runner nicht zuverlässig). Zusätzlich `webapp/supabase/_e2e_tests.sql` — ein self-asserting Integrationstest (61 Checks gegen die Live-Views, BEGIN/ROLLBACK), aufrufbar via `docker exec … psql … < supabase/_e2e_tests.sql`.
