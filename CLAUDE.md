...
- **PWA:** Service Worker (`public/sw.js`, `CACHE_VERSION = bordkasse-v3`) cached App-Shell; IndexedDB-Outbox erfasst Buchungen offline und synchronisiert beim Reconnect. Der `OfflineBanner` zeigt dauerhaft fehlschlagende Sync-Einträge mit „Erneut"/„Verwerfen" (statt eines ewig hängenden Zählers); Buchungsformulare retten zudem einen **Mid-Flight-Netzdrop** (offline NACH dem Klick) dedup-sicher in die Outbox.
...
**Tests:** Vitest (`__tests__/calc.test.ts` + `schema.test.ts` + `prepayments.test.ts` + `prepayment-dates.test.ts`) gegen S1–S7, das Spec-Szenario „Yachtanzahlung mit Kojen" sowie Edge-Cases (An-Bord-0-Anwesende, Pro-Person+Alkohol, An-Alle-N=2, `toCrewDueDate`-Clamp); Playwright (`e2e/smoke.spec.ts`) für öffentliche Routes + Auth-Schutz + Security-Header. **pgTAP** (`webapp/supabase/tests/*.sql`, ausführbar via `pnpm test:db` → `supabase test db`) prüft die echten SQL-Views/Funktionen (`v_transaction_shares`, `v_balances`, `simplify_debts`, `v_prepayment_payments`) inkl. Soft-Delete-Regression sowie die View-RLS-Härtung (`view_security_invoker_test.sql`: `security_invoker` gesetzt, `anon` ohne `SELECT`, nicht-Mitglied sieht 0 Zeilen) — **lokal lauffähig, bewusst NICHT im CI** (siehe entfernter db-lint-Job: `supabase start` läuft im GitHub-Runner nicht zuverlässig). Zusätzlich `webapp/supabase/_e2e_tests.sql` — ein self-asserting Integrationstest (61 Checks gegen die Live-Views, BEGIN/ROLLBACK), aufrufbar via `docker exec … psql … < supabase/_e2e_tests.sql`.
...
│   ├── __tests__/                       # Vitest (calc, schema, prepayments, prepayment-dates)
