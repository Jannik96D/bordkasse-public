...
- **Security:** RLS-Policies, Service-Role nur in Server Actions, Security-Header (HSTS/CSP/X-Frame/Referrer-Policy), `noindex`-Meta + `robots.txt`. **IDOR-Schutz im App-Layer (wichtig, da Service-Role RLS umgeht):** schreibende/löschende Actions filtern IMMER zusätzlich `trip_id` (nicht nur die Row-ID) — gilt für `deleteTransaction`, Kategorie-Actions und alle Buchungs-/Prepayment-Pfade inkl. `tranche_id`-Zugehörigkeit (`trancheBelongsToTrip`). Zusätzlich prüft `personsBelongToTrip` in `createExpense`/`updateExpense`/`createCredit`/`updateCredit`, dass `paid_by` / `participant_ids` / `credit_from` / `credit_to` wirklich Crew dieses Törns sind (Cross-Trip-Personen-Schutz; „Alle"=`null` wird übersprungen). **DB-Härtung:** Migration `0033_pin_search_path` nagelt `search_path = pg_catalog, public` auf alle SECURITY-DEFINER-Funktionen (`is_trip_member/-skipper`, `current_person_id`, `purge_*`, `delete_my_account`) + `all_debts_settled` fest; `0034_audit_log_rls` ersetzt die `audit_log`-SELECT-Policy durch `trip_id IS NOT NULL AND is_trip_skipper(trip_id)` (entfernt den offenen `trip_id IS NULL`-Lesezweig). **Magic-Link-Auth:** `resolveOrigin` akzeptiert den `Origin`-Header nur gegen eine Allowlist (sonst Redirect-Poisoning), `safeNextPath` schützt Post-Login-Redirects vor Open-Redirect. `lib/utils.ts:nowMs()` kapselt `Date.now()` (react-hooks/purity-Regel verbietet impure Calls in Komponenten/Effekten).
...
**Tests:** Vitest (`__tests__/calc.test.ts` + `schema.test.ts` + `prepayments.test.ts` + `prepayment-dates.test.ts`) gegen S1–S7, das Spec-Szenario „Yachtanzahlung mit Kojen" sowie Edge-Cases (An-Bord-0-Anwesende, Pro-Person+Alkohol, An-Alle-N=2, `toCrewDueDate`-Clamp); Playwright (`e2e/smoke.spec.ts`) für öffentliche Routes + Auth-Schutz + Security-Header. **pgTAP** (`webapp/supabase/tests/*.sql`, ausführbar via `pnpm test:db` → `supabase test db`) prüft die echten SQL-Views/Funktionen (`v_transaction_shares`, `v_balances`, `simplify_debts`, `v_prepayment_payments`) inkl. Soft-Delete-Regression — **lokal lauffähig, bewusst NICHT im CI** (siehe entfernter db-lint-Job: `supabase start` läuft im GitHub-Runner nicht zuverlässig). Zusätzlich `webapp/supabase/_e2e_tests.sql` — ein self-asserting Integrationstest (61 Checks gegen die Live-Views, BEGIN/ROLLBACK), aufrufbar via `docker exec … psql … < supabase/_e2e_tests.sql`.
...
│   │                                    # + tests/ (pgTAP: calc_views, balances_soft_delete) → pnpm test:db
