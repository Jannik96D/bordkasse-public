...
- **Törn-Fortschritt-Checkliste** (Migration 0029, reine Logik `lib/calc/trip-progress.ts` → `computeTripProgress`, Vitest-getestet): Karte „Dein Törn im Überblick" auf der Trip-Übersicht (**unter** den Schnellzugriff-Kacheln), sichtbar nur für Skipper/Co-Skipper/Admin (wie der Settlement-Banner). Bildet den gesamten Törn-Lebenszyklus in **5 Phasen** ab: Vorbereitung → Anzahlung (nur Charter) → Während des Törns → Abrechnung → Abschluss. **Fortschritts-Spiegel statt Todo-Liste:** alle Haken werden aus dem echten Datenstand abgeleitet (`lib/queries/trip-progress.ts` reused `getPlan`/`getPrepaymentPoolBalances`/`getCharterPaidTotal` + `getSimplifiedDebts`/`getSettledDebtKeys`; `all_debts_settled` in TS statt service-role-RPC) — nichts wird manuell abgehakt. Aktuelle Phase auto-aufgeklappt (erste nicht-erledigte), Items zukünftiger Phasen „noch nicht dran" (nie „offen/rot"), bei allem erledigt kompakte Abschluss-Zeile. Pro Mitglied minimierbar (`trip_members.checklist_collapsed_at`, DB-persistiert → geräteübergreifend; Server-Action `lib/actions/trip-checklist.ts`). a11y: `role=progressbar`, Status als Icon **und** Text (nicht nur Farbe/Emoji), Touch-Targets. Der Charter-Schalter `trips.has_charter_prepayment` (beim Törn-Anlegen + nachträglich in den Settings via `CharterSection`) blendet die Anzahlungs-Items ein; **OR-Fallback:** existiert bereits ein `prepayment_plan`, erscheinen sie ohnehin. „Alle Schulden beglichen" zählt erst nach Abrechnungs-Versand als erledigt (sonst zeigt ein Törn ohne Buchungen den Haken verfrüht). Verlinkt den Settlement-Banner, statt ihn zu duplizieren. Hinweis: überschneidet sich leicht mit dem kontextuellen Anzahlungs-CTA (`showCreatePrepaymentCta`) auf der Übersicht und der `PrepaymentPlanSection` in den Settings — bewusst koexistierend, nicht widersprüchlich.
