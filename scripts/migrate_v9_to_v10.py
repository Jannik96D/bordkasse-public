#!/usr/bin/env python3
"""
Bordkasse v9 → v10 Migration

Zwei Änderungen:
  1. Anwesenheits-Checkbox im Eingabe-Tab: Spalte D → Spalte C
  2. Apps Script: leere Crew-Slots (Besatzung B11:B22 leer) werden im
     Eingabe-Tab automatisch ausgeblendet — beim Öffnen, beim Toggle auf
     "Individuell" und live wenn in Besatzung Namen ergänzt/entfernt werden.

Aufruf:
  python3 scripts/migrate_v9_to_v10.py [SRC_XLSX]

Default-Quelle: assets/sheets-current/Bordkasse_IJsselmeer2026_v9.xlsx
Optional: anderen Pfad als CLI-Argument übergeben (z.B. modifizierte Datei
aus Downloads, in der Spaltenbreiten/Zeilenhöhen schon angepasst sind).

Erzeugt:
  assets/sheets-current/Bordkasse_IJsselmeer2026_v10.xlsx
  assets/sheets-current/Bordkasse_AppScript_v10.js
...
DEFAULT_SRC = SHEET_DIR / "Bordkasse_IJsselmeer2026_v9.xlsx"
DST_XLSX  = SHEET_DIR / "Bordkasse_IJsselmeer2026_v10.xlsx"
DST_JS    = SHEET_DIR / "Bordkasse_AppScript_v10.js"

# Anwesenheits-Block: Zeilen 31–42, alte Spalte D (4) → neue Spalte C (3)
DABEI_FIRST_ROW = 31
DABEI_LAST_ROW  = 42
OLD_COL = 4  # D
NEW_COL = 3  # C
...
def move_checkbox_column(ws):
    # Stil + Wert der alten Spalte D nach C übernehmen
    for r in range(DABEI_FIRST_ROW, DABEI_LAST_ROW + 1):
        old = ws.cell(r, OLD_COL)
        new = ws.cell(r, NEW_COL)
        clone_style(old, new)
        if old.value is not None:
            new.value = old.value
        # alte Zelle leeren (aber Style intakt lassen — Spalte D ist nur Spacer)
        old.value = None

    # Data-Validations auf Eingabe komplett neu setzen (alte D-DV verwerfen)
    target_dvs = []
    for dv in list(ws.data_validations.dataValidation):
        # "x"-DV auf D31:D42 wird auf C31:C42 umgemappt
        # andere DVs bleiben erhalten
        if dv.formula1 == '"x"':
            continue  # wird unten neu angelegt
        target_dvs.append(dv)

    ws.data_validations = DataValidationList()
    for dv in target_dvs:
        ws.add_data_validation(dv)

    new_dv = DataValidation(type="list", formula1='"x"', allow_blank=True)
    new_dv.add(f"C{DABEI_FIRST_ROW}:C{DABEI_LAST_ROW}")
...
def main():
    ...
    print("→ Verschiebe Anwesenheits-Checkbox D31:D42 → C31:C42")
    move_checkbox_column(wb["Eingabe"])

    print(f"→ Schreibe {DST_XLSX.relative_to(ROOT)}")
    wb.save(DST_XLSX)

    DST_JS.write_text(APPS_SCRIPT_V10, encoding="utf-8")
    print(f"✅ Apps Script geschrieben: {DST_JS.relative_to(ROOT)}")

    print("\n✅ Migration v9 → v10 abgeschlossen.")
    print("\nNächste Schritte:")
    ...
    print(f"  5. Test: Aufteilung auf 'Individuell', Crew-Slots ohne Namen sollten ausgeblendet sein.")
    print(f"           Namen in Besatzung B11:B22 ergänzen → Zeilen erscheinen automatisch.")
